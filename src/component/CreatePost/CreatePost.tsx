...
import { cancelPost } from 'pages/Blog/blog.slice'
...
function CreatePost() {
  ...
  const [formData, setFormData] = useState<Omit<Post, 'id'>>(initialFormData)
  const { data } = useGetPostIdQuery(postId, { skip: !Boolean(postId) })
  const [updatePost, updatePostResult] = useUpdatePostMutation()
  const [addPost, addPostResult] = useAddPostMutation()
  const dataPost = postId && data ? data : initialFormData
  ...
  return (
    <>
      <form onSubmit={handleSubmit} onReset={(e) => handleCancelPost()}>
        <div className="mb-6">
          <label
            htmlFor="title"
          ...
          <label
            ...
            className={classNames(
              'mb-2 block text-sm font-medium  dark:text-gray-300',
              {
                ...
                'text-red-900': Boolean(errorForm?.publishDate)
              }
            )}
          >
            Publish Date
